				...
				<div class="main-content">
					...
					<div class="mainform_box">
						<%
							...
							Statement st5=con.createStatement();
							String sql5="select * from mb_blog where b_id = "+b_id;
							ResultSet rs5=st5.executeQuery(sql5);
							if(rs5.next()){
						%>
						<form action="blog_edit_do.jsp?b_id=<%=b_id%>" method="post">
							<label>标题：</label>
							<p><input type="text" name="title" class="text" value="<%=rs5.getString("b_title")%>"/></p>
							...
							<p>
								...
								<%
									...
									String sql6="select * from mb_blog_type";
									ResultSet rs6=st6.executeQuery(sql6);
									while(rs6.next()){
								%>
									<option value="<%=rs6.getString("type_id")%>"
									<%if(rs6.getString("type_id").equals(rs5.getString("b_typeid"))){%>
									select<%}%>>
									<%=rs6.getString("type_name")%></option>
								<%
									}
									rs6.close();
									st6.close();
							...
							<label>摘要：</label>
							<p><textarea name="intro" class="textarea1"><%=rs5.getString("b_intro")%></textarea></p>
							
							<label>内容：</label>
							<p><textarea name="content" class="textarea2"><%=rs5.getString("b_content")%></textarea></p>
